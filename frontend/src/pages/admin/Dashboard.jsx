...
// }, [userRole]);
    ...
    const [selectedOfficeId, setSelectedOfficeId] = useState('');
      ...
      useEffect(() => {
        const fetchDatabaseRegistry = async () => {
            try {
                const response = await fetch(`http://localhost:5001/api/auth/employees?page=${currentPage}&limit=${rowsPerPage}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json'
                    },
    ...
    useEffect(() => {
        const fetchOffices = async () => {
            try {
                const res = await fetch('http://localhost:5001/api/branch?limit=1000', { credentials: 'include' });
                                            ...
                                            <div className="col-span-1 flex items-center justify-center gap-2">
                                                {/* <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${
                                                 ...
                                                 <button
                                                    ...
                                                    className={`w-10 h-5 flex items-center rounded-full p-0.5 transition-all duration-300 outline-none ${
                                                        (empStatus === 'active' || empStatus === undefined)
                                                ...
                                                >
                                                    ...
                                                    <div 
                                                        className={`w-3.5 h-3.5 rounded-full shadow-md transform transition-all duration-300 pointer-events-none ${
                                                            (empStatus === 'active' || empStatus === undefined)
                                    ...
                                    <div className="flex gap-2 w-full sm:w-auto justify-end">
                                        ...
                                        {getVisiblePageNumbers().map((pageNumber) => (
                                            <button
                                                ...
                                                className={`w-8 h-8 rounded-md border text-center transition-all font-semibold ${
                    ...
                    <div className="w-full max-w-sm bg-[#131B2E] border border-[#1E2943] rounded-2xl p-6 shadow-2xl relative overflow-hidden animate-in fade-in zoom-in-95 duration-150">
                        ...
                        <div className="py-4 space-y-3 text-xs">
                            ...
                            <div className="flex justify-between items-center bg-[#0B0F19]/30 p-2.5 rounded-lg border border-[#1E2943]/40">
                                ...
                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${
                                    selectedEmp.role === 'admin' ? 'bg-rose-500/10 border border-rose-500/20 text-rose-400' :
